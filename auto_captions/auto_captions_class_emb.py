...
def _clean_species_name(name: str) -> str:
    """
    Normalize species names so the prefix is consistent across training/inference.
    """
    if name is None:
        return "flower"
...
def make_natural_caption(
    ...
    """
    Canonical prefix first (always):
      'flower species: {name}. a photo of a flower.'
    Then optionally append the richer, image-derived description.
    """
    if rng is None:
        rng = random
    ...
    # ---- canonical prefix (ALWAYS) ----
    # Keep this EXACT format stable for best class binding.
    prefix = f"flower species: {species}. a photo of a flower."

    if not include_details:
        return prefix

    # ---- optional descriptive suffix (your original caption logic) ----
    ...
    suffix = tpl.format(shot=shot, colors=colors, flower=species, context=context, vibe=vibe)

    # Put the canonical label first, then natural language
    return prefix + " " + suffix

def tech_caption_from_steps(base_caption, steps):
    if not steps:
        return base_caption + ", no watercolor modules applied"
    names = ", ".join([s[0] for s in steps])
    desc = "; ".join([s[1] for s in steps])
