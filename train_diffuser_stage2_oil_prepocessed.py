...
from data_helpers.data_flowers_oil_preprocessed import Flowers102Oil
...
def train_stage2_oil(
    ...
    log_every=200,
    save_every=1000,
    data_dirname="jpg_oil",
):
    device = "cuda" if torch.cuda.is_available() else "cpu"
    os.makedirs(os.path.dirname(out) or ".", exist_ok=True)

    # dataset from preprocessed folder
    ds = Flowers102Oil(split="train", image_size=image_size, data_dirname=data_dirname)
    ...
    pbar = tqdm(total=steps_target, desc="Stage-2 oil painting fine-tune (v-pred)")
    global_step = 0
...
if __name__ == "__main__":
    train_stage2_oil(
        stage1_ckpt="ckpts/unet_stage1_clsemb_sd15.pt",
        vae_ckpt="ckpts/vae_512_lpips.pt",
        out="ckpts/oil_preproc/unet_stage2_oil.pt",
        data_dirname="jpg_oil",
    )
