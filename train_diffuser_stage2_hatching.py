...
from auto_captions.auto_captions_inksketch import make_inksketch_caption
from data_helpers.data_flowers_inksketch import Flowers102InkSketch
...
# ---------- procedural modules ----------
_sketch = InkSketchPipeline(
    freq=38.0,                 # less dense
    sharpness=10.0,            # thinner/softer lines
    thresholds=(0.42, 0.62, 0.78),  # only darker tones get extra layers
    angles=(45.0, -45.0, 0.0), # fewer layers than 4-angle cross hatch
    edge_strength=0.35,        # reduce outline dominance
    edge_threshold=0.18,       # fewer edges drawn
    paper_strength=0.05,
    use_edges=True,
)
...
def stylize_fn(img01):
    # randomize strengths each sample
    freq = random.uniform(32.0, 48.0)
    sharp = random.uniform(8.0, 12.0)
    edge_strength = random.uniform(0.20, 0.45)
    edge_thresh = random.uniform(0.16, 0.24)
    paper = random.uniform(0.03, 0.07)

    thresholds = (random.uniform(0.45, 0.55),
                  random.uniform(0.62, 0.72),
                  random.uniform(0.78, 0.88))

    pipeline = InkSketchPipeline(
        freq=freq,
        sharpness=sharp,
...
def train_stage2_poster(
    ...
    out="ckpts/unet_stage2_hatching.pt",
    image_size=512,
    bs=16,
    ...
    # ---- dataset ----
    ds = Flowers102InkSketch(split="train", image_size=image_size, sketch_fn=stylize_fn)
    ...
    while global_step < steps_target:
        ...
        # captions: canonical species + poster keywords
        caps = [make_inksketch_caption(names[i]) for i in range(len(names))]
