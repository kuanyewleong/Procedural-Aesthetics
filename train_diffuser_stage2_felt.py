...
from auto_captions.auto_captions_poster import make_poster_caption
from data_helpers.data_flowers_felt import Flowers102Felt
from procedural_modules.felt_modules import FeltPipeline
...
# ---------- procedural modules ----------
_felt = FeltPipeline(
    soften_k=9, soften_mix=0.85,
    shade_k=21, shade_amount=0.22,
    palette_levels=10,
    fiber_noise_strength=0.12,
    fiber_flow_strength=0.18,
    seam_strength=0.30,
)
...
def train_stage2_poster(
    ...
    # ---- dataset ----
    ds = Flowers102Felt(split="train", image_size=image_size, stylize_fn=stylize_fn)
    ...
    while global_step < steps_target:
        ...
        caps = [make_poster_caption(names[i]) for i in range(len(names))]
