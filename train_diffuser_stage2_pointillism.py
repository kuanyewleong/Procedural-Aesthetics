...
from data_helpers.data_flowers_pointillism import Flowers102Pointillism
from auto_captions.auto_captions_pointillism import make_pointillism_caption
from procedural_modules.pointillism_modules import PointillismPipeline
...
# ---------- procedural modules ----------
_point = PointillismPipeline(
    flatten=True,
    flatten_k=3,
    flatten_mix=0.35,
    contrast=0.35,
    layers=(
        dict(spacing=9, radius=4, jitter=0.95, softness=1.2, coverage=0.85, opacity=1.0, seed=1),
        dict(spacing=6, radius=3, jitter=0.95, softness=1.1, coverage=0.90, opacity=1.0, seed=2),
        dict(spacing=4, radius=2, jitter=0.98, softness=1.0, coverage=0.95, opacity=1.0, seed=3),
    ),
    ink_edges=False,  # add later if desired
)
...
def train_stage2_poster(
    ...
    out="ckpts/unet_stage2_pointillism.pt",
    image_size=512,
    bs=16,
    ...
    # ---- dataset ----
    ds = Flowers102Pointillism(split="train", image_size=image_size, pointillize_fn=stylize_fn)
    ...
    while global_step < steps_target:
        ...
        caps = [make_pointillism_caption(names[i]) for i in range(len(names))]
