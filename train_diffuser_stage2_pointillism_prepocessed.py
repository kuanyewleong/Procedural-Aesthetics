...
from data_helpers.data_flowers_pointillism_preprocessed import Flowers102Pointillism
...
def train_stage2_pointillism(
    stage1_ckpt="ckpts/unet_stage1_clsemb_sd15.pt",
    vae_ckpt="ckpts/vae_512_lpips.pt",
    out="ckpts/pointillism/unet_stage2_pointillism.pt",
    image_size=512,
    bs=16,
    lr=5e-5,
    wd=1e-2,
    ...
    log_every=200,
    save_every=1000,
    data_dirname="jpg_pointillism",
):
    device = "cuda" if torch.cuda.is_available() else "cpu"
    os.makedirs(os.path.dirname(out) or ".", exist_ok=True)

    # dataset from preprocessed folder
    ds = Flowers102Pointillism(split="train", image_size=image_size, data_dirname=data_dirname)
    ...
    pbar = tqdm(total=steps_target, desc="Stage-2 pointillism fine-tune (v-pred)")
    global_step = 0
