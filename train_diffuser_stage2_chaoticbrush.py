...
from data_helpers.data_flowers_chaoticbrush import Flowers102ChaoticBrushes
from auto_captions.auto_captions_chaoticbrush import make_chaotic_brushes_caption
...
# ---------- procedural modules ----------
_cbrush = ChaoticBrushesPipeline(
    tile_small=7,
    tile_big=14,
    n_brushes=180,
    min_length=8,
...
def train_stage2_poster(
    ...
    # ---- dataset ----
    ds = Flowers102ChaoticBrushes(split="train", image_size=image_size, stylize_fn=stylize_fn)
    ...
    while global_step < steps_target:
        ...
        # captions: canonical species + chaotic brushes keywords
        caps = [make_chaotic_brushes_caption(names[i]) for i in range(len(names))]
...
if __name__ == "__main__":
    train_stage2_poster(
        out="ckpts/chaoticbrush/unet_stage2_cbrush.pt"
    )
