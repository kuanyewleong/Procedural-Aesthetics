...
from data_helpers.data_flowers_postimp import Flowers102PostImpressionist
...
# ---------- procedural modules ----------
_postimp = PostImpressionistPipeline(
    smooth_k=11,
    smooth_mix=0.85,
    smooth_edge=12.0,
    sat=1.25,
    warm=0.06,
    stroke_strength=0.30,
    stroke_scale=6,
    palette_levels=8,
    palette_alpha=10.0,
    canvas_strength=0.06,
    canvas_scale=3,
)

@torch.no_grad()
def stylize_fn(img01):
    return _postimp(img01)


def train_stage2_poster(
    stage1_ckpt="ckpts/unet_stage1_clsemb_sd15.pt",
    vae_ckpt="ckpts/vae_512_lpips.pt",
    out="ckpts/unet_stage2_post-imp.pt",
    image_size=512,
    bs=16,
    ...
    # ---- dataset ----
    ds = Flowers102PostImpressionist(split="train", image_size=image_size, stylize_fn=stylize_fn)
    ...
    while global_step < steps_target:
        ...
        caps = [make_postimp_caption(names[i]) for i in range(len(names))]
