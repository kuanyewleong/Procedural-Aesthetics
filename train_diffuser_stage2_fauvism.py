...
from data_helpers.data_flowers_fauvism import Flowers102Fauvism
...
_fauve = FauvismPipeline(
    ...
    # emotion mapping (still strong)
    drift=0.10,
    drift_k=81,          # larger = smoother emotional color fields

    # complementary vibration (less blocky)
    comp_region_k=61,    # larger = smoother regions
    comp_strength=0.42,  # lower than 0.60

    # brushwork (less grid)
    n_dirs=5,
    stroke_len=8,
    stroke_strength=0.62,

    # texture (subtle)
    impasto_strength=0.08,
)
...
def train_stage2_poster(
    ...
    out="ckpts/unet_stage2_fauvism.pt",
    image_size=512,
    bs=16,
    ...
    # ---- dataset ----
    ds = Flowers102Fauvism(split="train", image_size=image_size, stylize_fn=stylize_fn)
    ...
    while global_step < steps_target:
        ...
        # captions: canonical species + fauvism keywords
        caps = [make_fauvism_caption(names[i]) for i in range(len(names))]
