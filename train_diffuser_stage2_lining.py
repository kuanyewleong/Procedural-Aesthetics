...
from data_helpers.data_flowers_lining import Flowers102Lining
...
def train_stage2_poster(
    ...
    # ---- dataset ----
    ds = Flowers102Lining(split="train", image_size=image_size, stylize_fn=stylize_fn)
    ...
    while global_step < steps_target:
        ...
        # captions: canonical species + lining keywords
        caps = [make_lining_caption(names[i]) for i in range(len(names))]
...
if __name__ == "__main__":
    train_stage2_poster(
        out="ckpts/lining/unet_stage2_lining.pt"
    )
