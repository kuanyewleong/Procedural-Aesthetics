...
from data_helpers.data_flowers_lowpoly import Flowers102LowPoly
from auto_captions.auto_captions_lowpoly import make_lowpoly_caption
...
# ---------- procedural modules ----------
_lowpoly = LowPolyPipeline(
    num_points=700,        # more points => finer triangles
    edge_quantile=0.90,    # 0.85 more edges, 0.95 fewer
    boundary_step=32,
    grid_step=64,           # try 64 for more uniform triangles
    edge_overlay=True,
    edge_strength=0.35,
    jitter=2,
    seed=0,
...
def train_stage2_poster(
    ...
    # ---- dataset ----
    ds = Flowers102LowPoly(split="train", image_size=image_size, stylize_fn=stylize_fn)
    ...
    while global_step < steps_target:
        ...
        # captions: canonical species + low-poly keywords
        caps = [make_lowpoly_caption(names[i]) for i in range(len(names))]
