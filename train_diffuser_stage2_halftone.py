...
from data_helpers.data_flowers_halftone import Flowers102Halftone
from auto_captions.auto_captions_halftone import make_halftone_caption
...
# ---------- procedural modules ----------
_halftone = HalftonePipelinePopArt(
    cell_size=9,        # smaller cells => more dots => more ink
    gamma=1.6,          # bigger dots in shadows
    max_radius=0.50,    # allow dots to fill cell more
    softness=0.9,       # crisper dots
    edge_strength=0.7  # stronger outlines
)


@torch.no_grad()
def stylize_fn(img01):
    return _halftone(img01)


def train_stage2_poster(
    stage1_ckpt="ckpts/unet_stage1_clsemb_sd15.pt",
    vae_ckpt="ckpts/vae_512_lpips.pt",
    out="ckpts/unet_stage2_halftone.pt",
    image_size=512,
    bs=16,
    ...
    # ---- dataset ----
    ds = Flowers102Halftone(split="train", image_size=image_size, halftone_fn=stylize_fn)
    ...
    opt = torch.optim.AdamW(
        [
            ...
            {"params": ctx_adapter.parameters(), "lr": 5e-5},
            {"params": class_embed.parameters(), "lr": 5e-5},
        ],
        weight_decay=1e-2,   # shared across groups unless overridden per-group
    )
    ...
    while global_step < steps_target:
        ...
        caps = [make_halftone_caption(names[i]) for i in range(len(names))]
