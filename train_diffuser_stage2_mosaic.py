...
from data_helpers.data_flowers_mosaic import Flowers102Mosaic
from auto_captions.auto_captions_mosaic import make_mosaic_caption
...
_mosaic = MosaicPipeline(
    ...
    jitter_color=0.015,
    grout=1,
    grout_color=(0.06, 0.06, 0.06),
)

@torch.no_grad()
def stylize_fn(img01):
    return _mosaic(img01)
...
def train_stage2_poster(
    ...
    # ---- dataset ----
    ds = Flowers102Mosaic(split="train", image_size=image_size, stylize_fn=stylize_fn)
    ...
    while global_step < steps_target:
        ...
        # captions: canonical species + mosaic keywords
        caps = [make_mosaic_caption(names[i]) for i in range(len(names))]
...
if __name__ == "__main__":
    train_stage2_poster(
        out="ckpts/unet_stage2_mosaic.pt"
    )
