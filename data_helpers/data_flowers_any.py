...
class Flowers102Any(Dataset):
    """
    Reads Oxford-102 split/labels from .mat, but loads images from:
      {flowers_root}/jpg_any/image_XXXXX.jpg
    """
    def __init__(self, root="./data", split="train", image_size=512, seed=0, any_dirname="jpg_any"):
        assert split in ["train", "val", "test"]
        self.root = Path(root)
        self.flowers_root = _find_flowers102_root(self.root)
        self.any_dir = self.flowers_root / any_dirname
        if not self.any_dir.exists():
            raise FileNotFoundError(f"Any folder not found: {self.any_dir}")
    ...
    def __getitem__(self, idx) -> Dict[str, Any]:
        ...
        # any file path must match original naming
        path = self.any_dir / f"image_{real_idx+1:05d}.jpg"
