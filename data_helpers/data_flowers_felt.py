...
from auto_captions.auto_captions_felt import make_felt_caption

class Flowers102Felt(Dataset):
    """
    Stage-2 felt dataset:
      - base image from Flowers102Clean
      - run felt stylizer
      - caption includes canonical species prefix + felt style keywords
    """
    def __init__(
        self,
        root="./data",
    ...
    def __getitem__(self, idx):
        ...
        # apply felt stylization (must return [3,H,W] in [0,1])
        img01 = self.stylize_fn(img01).clamp(0.0, 1.0)

        # canonical felt caption based on true species
        name = b["name"]
        caption = make_felt_caption(name, rng=self.rng)
