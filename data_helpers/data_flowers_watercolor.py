...
class Flowers102Watercolor(Dataset):
    """
    Reads Oxford-102 split/labels from .mat, but loads images from:
      {flowers_root}/jpg_watercolor/image_XXXXX.jpg
    """
    def __init__(self, root="./data", split="train", image_size=512, seed=0, watercolor_dirname="jpg_watercolor"):
        assert split in ["train", "val", "test"]
        self.root = Path(root)
        self.flowers_root = _find_flowers102_root(self.root)
        self.water_dir = self.flowers_root / watercolor_dirname
        if not self.water_dir.exists():
            raise FileNotFoundError(f"Watercolor folder not found: {self.water_dir}")
